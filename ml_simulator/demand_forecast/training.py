...
from demand_forcast.model import MultiTargetModel
...
def extract_features(
    ...
    from demand_forcast.features import add_features, add_targets
    ...
    df_features.sort_values(["sku_id", "day"], inplace=True) # << remove?
    print(f"Features extracted. features.csv shape: {df_features.shape}")

    mask = df_features.isna().sum(axis=1)==0
...
def split_train_test(
    ...
    from demand_forcast.model import split_train_test
...
def fit_model(
    ...
    model.fit(df_features, verbose=False)
...
def evaluate(
    ...
    from demand_forcast.evaluate import evaluate_model

    print("Evaluating model...")

    predictions = eval_model.predict(df_test)
    predictions = predictions.sort_values(['day', 'sku_id'])
    losses = evaluate_model(df_test, predictions)
...
def deploy_model(
    ...
    import pickle
    from demand_forcast.evaluate import plausibility_test

    print("Check model quality...")

    print(f"Losses: {losses}")

    plausibility_test(losses)
