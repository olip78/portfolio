...
def select_suitable_clients(engine: Engine, n_clients: int, date_to: int) -> dd.DataFrame:
    receipts = engine.get_table('receipts')
    """selects subset of clients for the next marketing campaign

    Args: 
        n_clients: number of clients
        date_to: max date of the last purchase

    Returns:
        clients: clients subset
    """

    date_from = date_to - N_DAYS_CHURN
    date_mask = (receipts['date'] >= date_from) & (receipts['date'] < date_to)

    clients = (
        receipts
        .loc[date_mask]['customer_id']
        .unique().compute()
        .sample(n=n_clients)
...
def create_transform_pipeline(config: list, artifacts_root_path: str) -> skpipe.Pipeline:
    """build feature transformations pipeline on the base of config
    """
    steps = list()
...
class CampaignFlow(Flow):
    """
    The class for starting the calculations needed to start the campaign. 
    
    * It is parameterized by the "big" config.
    * Calculation consists of several stages.
    * It is possible to restart the calculation from the last successful stage.
    * Saves intermediate results to the hard disk.
    
    Args:
        engine: database connection.
        config: calculation configuration.
        filenames: dictionary of file names for intermediate results.
        run_id: calculation id. Intermediate results are restored by it when the process is re-raised.
        runs_root_path: folder where a subfolder will be created for the calculation.
        artifacts_root_path: the path relative to which the artifacts paths are set in config. 
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.date_to = self.config['date_to']
    ...
    def _run_stage_select_suitable_clients(self):
        """Initial selection of clients for the campaign.
        """
    ...
    def _run_stage_extract_features(self):
        """Collection of features from sources.
        """
    ...
    def _run_stage_transform_features(self):
        '''The second step of factor calculation. A transformation of the factors takes place.
        '''
        _raw_features = dd.from_pandas(self.clients, npartitions=1).merge(
            self.raw_features, on=['customer_id'], how='left'
        ).compute()
    ...
    def _run_stage_recommend(self):
        ...
        for group_idx, group in enumerate(self.client_groups):
            ...
            if group_params['treatment_params']['type'] == 'constant':
                ...
                part = (
                    part
                    .merge(self.features[['customer_id', col_treatment]], on=['customer_id'], how='left')
