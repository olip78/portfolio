...
def create_engine(data_root_path: str) -> Engine:
    engine = Engine(tables={
        ...
        'campaigns': pd.read_csv(os.path.join(data_root_path, 'campaigns.csv')),
        'customers': pd.read_csv(os.path.join(data_root_path, 'customers.csv')),
    })
    return engine
...
if __name__ == '__main__':
    ...
    args_parser.add_argument("--date-to", required=True, type=int)
    ...
    config['date_to'] = args.get('date_to')
